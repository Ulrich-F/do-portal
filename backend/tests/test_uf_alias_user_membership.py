...
#create rootuser
#add membership too rootuser
#create aliasuser
#=> auf API seite sollte normaler user nochmal angelegt werden
#add membership to aliasuser
#=> with sub-organisations
...
def test_create_rootuser(client):
    #test user noch nicht angelegt
    assert User.query.filter_by(_name=TAU.rootuser_name).first() == None, 'User found via query before insert.'
    ...
    #test user authenticate moeglich
    (admin, auth) = User.authenticate(TAU.rootuser_email, TAU.rootuser_password)
    assert auth is True, 'User cant log in'

    #test zugriff nur auf einen user, da neue org und dazu neuer user
    assert len(admin.get_users()) == 1, 'Falsche Anzahl User zum neuen User'


def test_create_aliasuser(client):
    user = User.query.filter_by(_name=TAU.rootuser_name).first()

    #mark: vorgesehen, dass org.id uebergeben werden kann, da ist aber nix implementiert.
...
def test_get_organizations(client):
    (admin, auth) = User.authenticate(TAU.rootuser_email, TAU.rootuser_password)
    assert auth is True, 'User cant log in'
    assert admin.get_organizations().count() == 5, 'Falsche Anzahl Organizations zum neuen User'
...
def test_get_organizations_deleted(client):
    ...
    (admin, auth) = User.authenticate(TAU.rootuser_email, TAU.rootuser_password)
    assert auth is True, 'User cant log in'
    assert admin.get_organizations().count() == 4, 'Falsche Anzahl Organizations zum neuen User'
...
def test_get_memberships(client):
    (admin, auth) = User.authenticate(TAU.rootuser_email, TAU.rootuser_password)
    assert auth is True, 'User cant log in'
    assert admin.get_memberships().count() == 4, 'Falsche Anzahl Organizations zum neuen User'
